...
namespace KnowledgeBox.Custom
{
    public static class LuceneSearch
    {
        ...
        private static KnowledgeBox.Models.Item _mapLuceneDocumentToData(Document doc)
        {
            return GetItem(Convert.ToInt32(doc.Get("Id")));
            //return new KnowledgeBox.Models.Item
            //{
            //    Item_Id = Convert.ToInt32(doc.Get("Id")),
            //    Item_Name = doc.Get("Name"),
            //    Item_Description = doc.Get("Description"),
            //    Item_TagWords = doc.Get("Tags"),
            //    Item_Date = Convert.ToDateTime(doc.Get("Date"))                            };
        }
        private static KnowledgeBox.Models.Item GetItem(int id)
        {
            return new KnowledgeBoxEntities().Items.Find(id);
        }
        private static IEnumerable<KnowledgeBox.Models.Item> _mapLuceneToDataList(IEnumerable<Document> hits)
        {
        ...
        private static IEnumerable<KnowledgeBox.Models.Item> _search(string searchQuery, string searchField = "")
        {
            // validation
            if (string.IsNullOrEmpty(searchQuery.Replace("*", "").Replace("?", ""))) return new List<KnowledgeBox.Models.Item>();
            ...
            using (var searcher = new IndexSearcher(_directory, false))
            {
                ...
                {
                    var parser = new MultiFieldQueryParser(Lucene.Net.Util.Version.LUCENE_30, new[] { "Id", "Name", "Description", "Tags" }, analyzer);
                    var query = parseQuery(searchQuery, parser);
        ...
        public static IEnumerable<KnowledgeBox.Models.Item> Search(string input, string fieldName = "")
        {
            if (string.IsNullOrEmpty(input)) return new List<KnowledgeBox.Models.Item>();
        ...
        public static void AddDataToLuceneIndex()
        {
            LuceneSearch.AddUpdateLuceneIndex(new KnowledgeBoxEntities().Items);
        }
